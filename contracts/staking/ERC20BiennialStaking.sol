...
contract ERC20BiennialStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 private constant LOCK_PERIOD = 730 days; // dois anos para o bloqueio
    uint256 private constant INITIAL_REWARD_RATE = 175; // 1.75% em base de pontos percentuais
    uint256 private constant FINAL_REWARD_RATE = 25; // 0.25% em base de pontos percentuais
    uint256 private constant MAX_STAKE = 4000000 * (10**18); // limite de 4.000.000 tokens (com 18 casas decimais)
    uint256 private constant REWARD_FEE_RATE = 2; // 2% taxa sobre a recompensa
    uint256 private constant REWARD_THRESHOLD = 30000 * (10**18); // Dividendos 30.000 - Distribuição para os Holders
    uint256 private constant MAX_REWARD_PERIOD = 910 days; // Período máximo de recompensa para o recebimento de recompensa
    ...
    function stake(uint256 amount) public notFrozen nonReentrant whenNotPaused {
        require(amount >= 1000, "Minimum staking amount is 1000 tokens"); 
