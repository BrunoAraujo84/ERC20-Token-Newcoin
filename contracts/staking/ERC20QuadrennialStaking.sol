...
contract ERC20QuadrennialStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 private constant LOCK_PERIOD = 1460 days; // quatro anos para o bloqueio
    uint256 private constant INITIAL_REWARD_RATE = 250; // 2.5% em base de pontos percentuais
    uint256 private constant FINAL_REWARD_RATE = 25; // 0.25% em base de pontos percentuais
    uint256 private constant MAX_STAKE = 6000000 * (10**18); // limite de 6.000.000 tokens (com 18 casas decimais)
    uint256 private constant REWARD_FEE_RATE = 5; // 5% taxa sobre a recompensa
    uint256 private constant REWARD_THRESHOLD = 50000 * (10**18); // Dividendos 50.000 - Distribuição para os Holders
    uint256 private constant MAX_REWARD_PERIOD = 1640 days; // Período máximo de recompensa para o recebimento de recompensa
    ...
    function stake(uint256 amount) public notFrozen nonReentrant whenNotPaused {
        require(amount >= 2000, "Minimum staking amount is 2000 tokens"); 
